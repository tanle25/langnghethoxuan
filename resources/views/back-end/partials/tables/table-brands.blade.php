	...
	<tbody>
		...
		<tr>
			...
			<td class="text-center">
                <img src="{{asset($obj->url)}}" style="max-width: 100px;">
            </td>
            <td class="text-center">
                {{$obj->name}}
                {{-- @dd($obj) --}}
			...
			<td class="text-center">
				<a href="{{route('brand_edit', ['id'=>$obj->id])}}" class="btn btn-warning btn-circle"><i class="fa fa-edit"></i></a>
				<a 	class="btn btn-danger btn-circle btn-sm delete-button"
					data-action ="{{ route('brand_delete',$obj->id) }}" type="button">
